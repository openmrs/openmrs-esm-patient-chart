...
export function buildFavoriteOrder(
  ...
  if (isSpecificFavorite) {
    return {
      ...
      conceptUuid: drugForSave?.concept?.uuid ?? conceptUuid,
      conceptName: drugForSave?.concept?.display ?? conceptName,
      displayName: computedName,
      attributes,
    };
  } else {
    if (!conceptUuid) {
      throw new Error('Cannot create concept-based favorite: no concept UUID available.');
    }
    return {
      id,
      conceptUuid,
...
export function validateConceptAvailable(isSpecificFavorite: boolean, conceptUuid?: string): boolean {
  return isSpecificFavorite || Boolean(conceptUuid);
}
