...
import { match, useHistory, useRouteMatch, BrowserRouter } from 'react-router-dom';
...
describe('<ProgramsForm />', () => {
  ...
  it('renders the add program form with all the appropriate fields and values', async () => {
    ...
    mockCreateProgramEnrollment.mockReturnValue(of({ status: 201, statusText: 'Created' }));
    ...
    expect(screen.getByText('Oncology Screening and Diagnosis').textContent).toBeTruthy();
    expect(screen.getByText('HIV Differentiated Care').textContent).toBeTruthy();
    ...
    expect(screen.getByText('Mosoriot Subcounty Hospital').textContent).toBeTruthy();
  ...
  it('renders the edit program form when the edit button is clicked on an existing program', async () => {
    ...
    mockUseRouteMatch.mockReturnValue(testMatch);
    mockUpdateProgramEnrollment.mockReturnValue(of({ status: 200, statusText: 'OK' }));
    ...
    expect(screen.getByLabelText('Program')).toBeInTheDocument();
    expect(screen.getByText('Oncology Screening and Diagnosis')).toBeInTheDocument();
    expect(screen.getByLabelText('Date enrolled')).toBeInTheDocument();
    expect(screen.getByDisplayValue(renderDate(testMatch.params.enrollmentDate))).toBeInTheDocument();
    expect(screen.getByLabelText('Date completed')).toBeInTheDocument();
    expect(screen.getByDisplayValue(renderDate(testMatch.params.completionDate))).toBeInTheDocument();
    expect(screen.getByLabelText('Enrollment location')).toBeInTheDocument();
    const cancelBtn = screen.getByRole('button', { name: 'Cancel' });
    expect(cancelBtn).toBeInTheDocument();
    expect(cancelBtn).not.toBeDisabled();
  ...
  it('displays an error when an invalid enrollment date is selected', async () => {
    ...
    await screen.findByText('Please enter a date that is either on or before today.');
