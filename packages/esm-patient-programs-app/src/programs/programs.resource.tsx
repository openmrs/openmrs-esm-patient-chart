...
export function fetchEnrolledPrograms(patientID: string) {
  return openmrsObservableFetch<PatientProgram>(`/ws/rest/v1/programenrollment?patient=${patientID}`).pipe(
    map(({ data }) => data['results'].sort((a, b) => (b.dateEnrolled > a.dateEnrolled ? 1 : -1))),
  );
}

export function fetchActiveEnrollments(patientID: string) {
  return openmrsObservableFetch<Array<PatientProgram>>(`/ws/rest/v1/programenrollment?patient=${patientID}`).pipe(
    map(({ data }) =>
