...
import { AllergyData, AllergicReaction, Allergen, DataCaptureComponentProps } from '../types';
...
const AllergyForm: React.FC<AllergyFormProps> = ({
  ...
  const [allergicReactions, setAllergicReactions] = useState<Array<AllergicReaction>>([]);
  const [selectedAllergicReactions, setSelectedAllergicReactions] = useState<Array<SelectedAllergicReaction>>([]);
  ...
  const [selectedAllergyCategory, setSelectedAllergyCategory] = useState<string>(null);
  const [reactionSeverityUuid, setReactionSeverityUuid] = useState<string>(null);
  ...
  useEffect(() => {
    if (isEditFormActive) {
      const getAllergicReactionsSub = getAllergicReactions().subscribe(
        (allergicReactions: Array<AllergicReaction>) => setAllergicReactions(allergicReactions),
        createErrorHandler(),
      );
      return () => {
  ...
  useEffect(() => {
    if (selectedAllergyCategory && !isEditFormActive) {
      const getAllergensSub = getAllergyAllergenByConceptUuid(selectedAllergyCategory).subscribe(
  ...
  const handleAllergicReactionChange = (checked: boolean, id: string, event: React.ChangeEvent<HTMLInputElement>) => {
    const eventTarget = event.currentTarget;
    setSelectedAllergicReactions((reactions: Array<SelectedAllergicReaction>) => {
      if (checked === true) {
        reactions.push({ uuid: eventTarget.value });
        return reactions;
      } else {
        return reactions.filter((reaction) => reaction.uuid !== eventTarget.value);
      }
    });
  };
  ...
  const handleEditFormSubmit = (event: SyntheticEvent<HTMLFormElement>) => {
    ...
    const abortController = new AbortController();
    updatePatientAllergy(allergy, patientUuid, match.params, abortController).then((response) => {
  ...
  const allergyHasReaction = (uuid: string) => {
    return patientAllergy?.reactions?.some((reaction) => reaction?.reaction?.uuid === uuid);
  };

  const handleDeletePatientAllergy = () => {
    const abortController = new AbortController();
    deletePatientAllergy(patientUuid, match.params, abortController).then((response) => {
    ...
    return (
      <SummaryCard
        ...
        <form
          ...
          <h4 className={`${styles.allergyHeader}`}>{t('reactionCategory', 'Category of reaction')}</h4>
          <div className={`${styles.container}`}>
            <RadioButtonGroup
              ...
              <RadioButton labelText={t('drug', 'Drug')} value={AllergyConcept.DRUG_ALLERGEN} />
              <RadioButton labelText={t('food', 'Food')} value={AllergyConcept.FOOD_ALLERGEN} />
              <RadioButton
              ...
              <RadioButton
                id="no-allergies"
                labelText={t('noKnownAllergies', 'Patient has no known allergies')}
                value="noAllergy"
              />
            </RadioButtonGroup>
          </div>
          {allergensArray && (
            <div>
              <h4 className={`${styles.allergyHeader}`}>
                {capitalize(getAllergyType(selectedAllergyCategory)?.toLowerCase())} {t('allergen', 'Allergen')}
              </h4>
              <div className={styles.container}>
                <RadioButtonGroup
                  labelPosition="right"
          ...
          {allergensArray && allergicReactions && (
            <div>
              <h4 className={`${styles.allergyHeader}`}>{t('reactions', 'Reactions')}</h4>
            ...
            <div>
              ...
              <div className={styles.container}>
                <RadioButtonGroup
                  ...
                  <RadioButton id="mild" labelText={t('mild', 'Mild')} value={AllergyConcept.MILD_REACTION_SEVERITY} />
                  <RadioButton
                    id="moderate"
              ...
              <h4 className={`${styles.allergyHeader}`}>
                <label htmlFor="first-onset-date">{t('firstOnsetDate', 'Date of first onset')}</label>
              </h4>
              <div className={styles.dateContainer}>
                <DatePicker dateFormat="m/d/Y" datePickerType="single" maxDate={new Date().toUTCString()}>
                  <DatePickerInput
              ...
              <h4 className={`${styles.allergyHeader}`}>{t('comments', 'Comments')}</h4>
          ...
          <div
            className={
              enableCreateButtons ? styles.buttonStyles : `${styles.buttonStyles} ${styles.buttonStylesBorder}`
            }
            style={{ position: 'sticky' }}>
            <Button onClick={closeForm} kind="secondary">
              {t('cancel', 'Cancel')}
            </Button>
            <Button type="submit" disabled={!enableCreateButtons} kind="primary">
      ...
      <SummaryCard
        ...
        {patientAllergy && allergicReactions?.length && (
          <form
            ...
            <div>
              <div className={`${styles.allergyEditHeader} omrs-padding-bottom-28`}>
                <h4>{t('allergen', 'Allergen')}</h4>
                <h3>
                  {patientAllergy?.allergen?.codedAllergen?.display}{' '}
                  <span>({patientAllergy?.allergen?.allergenType?.toLowerCase()})</span>
                </h3>
              </div>
              <div>
                <h4 className={`${styles.allergyHeader}`}>{t('reactions', 'Reactions')}</h4>
                <h4 className={`${styles.allergyHeader}`}>{t('selectAllThatApply', 'Select all that apply')}</h4>
                <div className={styles.container}>
                  {allergicReactions.map((reaction, index) => (
                    <Checkbox
                      key={index}
                ...
                <div className={styles.dateContainer}>
                  <DatePicker dateFormat="m/d/Y" datePickerType="single">
                    <DatePickerInput
                      ...
                      placeholder="mm/dd/yyyy"
                      type="text"
                      defaultValue={dayjs(patientAllergy?.auditInfo?.dateCreated).format('MM/DD/YYYY')}
            ...
            <div
              ...
              <Button onClick={handleDeletePatientAllergy} kind="danger" style={{ width: '20%' }}>
                {t('delete', 'Delete')}
              </Button>
              <Button onClick={closeForm} kind="secondary" style={{ width: '30%' }}>
                {t('cancel', 'Cancel')}
              </Button>
              <Button type="submit" kind="primary" disabled={!enableEditButtons} style={{ width: '50%' }}>
                {t('signAndSave', 'Sign & Save')}
              </Button>
            </div>
          </form>
  ...
  return <div className={styles.allergyForm}>{isEditFormActive ? editAllergy() : createAllergy()}</div>;
};
