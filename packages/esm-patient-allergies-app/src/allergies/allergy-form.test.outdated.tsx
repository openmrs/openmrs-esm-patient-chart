...
describe('<AllergyForm />', () => {
  ...
  beforeEach(() => {
    ...
    mockGetAllergicReactions.mockReturnValue(of(mockAllergicReactions.setMembers));
    mockGetAllergyAllergenByConceptUuid.mockReturnValue(of(mockEnvironmentalAllergens));
  });

  it('renders the create allergy form with the appropriate fields and values', async () => {
    mockSavePatientAllergy.mockReturnValue(Promise.resolve(mockSaveAllergyResponse));
    ...
    expect(screen.getByLabelText('Patient has no known allergies')).toBeInTheDocument();

    const environmentalAllergenCheckbox = screen.getByLabelText('Environmental');
    ...
    expect(screen.getByRole('checkbox', { name: 'Mental status change' })).not.toBeChecked();

    fireEvent.click(screen.getByRole('checkbox', { name: 'Mental status change' }));

    expect(screen.getByRole('checkbox', { name: 'Mental status change' })).toBeChecked();
  ...
  it('renders the edit allergy form with the relevant form fields prefilled', async () => {
    ...
    mockGetPatientAllergyByPatientUuid.mockResolvedValue(Promise.resolve(mockAllergyResult));
    mockGetAllergicReactions.mockReturnValue(of(mockAllergicReactions.setMembers));
    ...
    mockUpdatePatientAllergy.mockReturnValue(Promise.resolve(mockUpdatedAllergyResult));
    ...
    expect(screen.getByText(/ARBs \(angiotensin II receptor blockers\)/)).toBeInTheDocument();
