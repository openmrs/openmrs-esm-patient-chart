    ...
    const filledForms = forms.map((form) => {
      completedForms.map((completeForm) => {
        if (completeForm.uuid === form.uuid) {
          form.complete = true;
          form.lastCompleted = completeForm.lastCompleted
            ? completeForm.lastCompleted
            : null;
        }
      });
      return form;
      ...
      <div className={styles.formsWidgetContainer}>
        ...
        <div style={{ width: "100%" }}>
          ...
          {selectedFormView === FormViewState.recommended && (
            <EmptyFormView
              action={t(
                "noRecommendedFormsAvailable",
                "No recommended forms available at the moment"
              )}
            />
          )}
        </div>
