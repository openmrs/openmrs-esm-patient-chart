...
import { mockEncounterResponse, mockAlternativeEncounterResponse } from '../../../../__mocks__/encounters.mock';
...
describe('<NoteRecord />', () => {
  ...
  it('displays a detailed summary of the selected note', async () => {
    ...
    expect(
      screen.getByText('Visit Diagnoses: Presumed diagnosis, Primary, Vitamin A Deficiency with Keratomalacia'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Visit Diagnoses: Other disease of hard tissue of teeth, Secondary, Confirmed diagnosis'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Text of encounter note: Duis aute irure dolor in reprehenderit in voluptat'),
    ).toBeInTheDocument();
  });
  ...
  it('displays the patient note details when present', async () => {
    mockFetchPatientEncounter.mockReturnValue(of(mockAlternativeEncounterResponse));
    ...
    expect(screen.getByText('Systolic blood pressure: 120.0')).toBeInTheDocument();
    expect(screen.getByText('Pulse: 60.0')).toBeInTheDocument();
    expect(screen.getByText('Blood oxygen saturation: 92.0')).toBeInTheDocument();
    expect(screen.getByText('Temperature (C): 37.0')).toBeInTheDocument();
    expect(screen.getByText('Diastolic blood pressure: 80.0')).toBeInTheDocument();
  });
});
