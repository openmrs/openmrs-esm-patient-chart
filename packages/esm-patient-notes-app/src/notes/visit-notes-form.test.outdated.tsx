...
describe('Visit notes form', () => {
  ...
  it('displays a message when no matching diagnosis are found', async () => {
    ...
    const spanElement = await screen.findByText('No matching diagnoses have been found');
  ...
  it("clicking 'Save & Close' submits the form ", async () => {
    ...
    expect(mockSaveVisitNote).toHaveBeenCalledWith(new AbortController(), jasmine.objectContaining(testPayload));
  });
});
