...
describe('useOrderBasket', () => {
  ...
  it('can modify items in one grouping without affecting the other', () => {
    const { result: drugResult } = renderHook(() => useOrderBasket('medications', ((x) => x) as unknown as PostDataPrepFunction));
    const { result: labResult } = renderHook(() => useOrderBasket('labs', ((x) => x) as unknown as PostDataPrepFunction));
