...
export function PrintComponent({ formName, patientDetails }: PrintComponentProps) {
  ...
  return (
    <div className={styles.printPage}>
      <header className={styles.header}>
        {printOptions?.logo?.src ? (
          <img src={interpolateUrl(printOptions?.logo.src)} alt={printOptions?.logo.alt} width={110} height={40} />
        ) : printOptions?.logo?.name ? (
          printOptions?.logo.name
        ) : (
