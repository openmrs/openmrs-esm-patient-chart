...
export default function AllergyCard(props: AllergyCardProps) {
  ...
  React.useEffect(() => {
    performPatientAllergySearch(props.currentPatient.identifier[0].value)
  ...
  return (
    <SummaryCard name="Allergy" match={props.match}>
      {patientAllergy &&
        ...
        patientAllergy.entry.map((allergy, index) => {
          return (
            <SummaryCardRow key={index} linkTo="/">
              <SummaryCardRowContent justifyContent="space-between">
                <span className={`omrs-bold ${style.left}`}>
                  {allergy.resource.code.text}
                </span>
                <span className={`${style.right} ${style.allergyReaction}`}>
