...
test('Record Vitals of the patient', async ({ page, api }) => {
  ...
  await test.step('Then I should see the vitals record', async () => {
    ...
    await expect(vitalsPage.vitalsTable().getByRole('cell', { name: '37' , exact:true})).toBeVisible();
    await expect(vitalsPage.vitalsTable().getByRole('cell', { name: '80', exact: true })).toBeVisible();
    await expect(vitalsPage.vitalsTable().getByRole('cell', { name: '120 / 80', exact: true })).toBeVisible();
    await expect(vitalsPage.vitalsTable().getByRole('cell', {name: '98', exact:true})).toBeVisible();
    await expect(vitalsPage.vitalsTable().getByRole('cell', { name: '20 ↑', exact:true })).toBeVisible();
  });
});
