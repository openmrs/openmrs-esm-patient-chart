...
test('Add, edit and delete biometrics', async ({ page }) => {
  const vitalsPage = new BiometricsAndVitalsPage(page);
  const headerRow = vitalsPage.biometricsTable().locator('thead > tr');
  const dataRow = vitalsPage.biometricsTable().locator('tbody > tr');
  ...
  await test.step('And I click the `Record biometrics` link to launch the form', async () => {
    await vitalsPage.page.getByText(/record biometrics/i).click();
  });
  ...
  await test.step('When I fill `40` as the height', async () => {
    await vitalsPage.page.getByRole('spinbutton', { name: /height/i }).fill('40');
  });

  await test.step('And I fill `40` as the weight', async () => {
    await vitalsPage.page.getByRole('spinbutton', { name: /weight/i }).fill('40');
  });

  await test.step('And I fill `30` as the muac', async () => {
    await vitalsPage.page.getByRole('spinbutton', { name: /muac/i }).fill('30');
  });
  ...
  await test.step('And I should see the newly recorded biometrics on the page', async () => {
    ...
    await expect(dataRow).toContainText('30');
  });
  ...
  await test.step('And I change the `Weight`', async () => {
    await vitalsPage.page.getByRole('spinbutton', { name: /weight/i }).clear();
    await vitalsPage.page.getByRole('spinbutton', { name: /weight/i }).fill('42');
  });
  ...
  await test.step('And I should see the newly edited biometrics on the page', async () => {
    ...
    await expect(dataRow).toContainText('42');
    await expect(dataRow).toContainText('40');
    await expect(dataRow).toContainText('262.5');
    await expect(dataRow).toContainText('30');
  });
  ...
  await test.step('And the biometrics table should be empty', async () => {
    await expect(vitalsPage.page.getByText(/there are no biometrics to display for this patient/i)).toBeVisible();
  });
});
